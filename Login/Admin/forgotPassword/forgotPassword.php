...
if (isset($_POST['email'])) {
    ...
    if (mysqli_num_rows($results) > 0) {
        ...
        $sql = "UPDATE adminlogin SET token='$token'WHERE email='$email'";
        $results = mysqli_query($db, $sql);

        $sql = "UPDATE adminlogin SET tokenExpire=DATE_ADD(NOW(), INTERVAL 5 MINUTE WHERE email='$email')";
        $results = mysqli_query($db, $sql);
        ...
        $mail->isHTML(true);
        $mail->Body = "
                Hi, dear user<br><br>It appears that you have requested for password reset.<br>In order to reset your password, click on the link below:
                <a href='http://localhost/AD%20Project%20System/Login/Admin/forgotPassword/resetPassword.php?email=$email&token=$token'><br>Reset Password Link</a><br><br>
