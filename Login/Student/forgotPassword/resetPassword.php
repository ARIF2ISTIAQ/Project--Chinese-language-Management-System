...
if (isset($_GET['email']) && isset($_GET['token'])) {
    ...
    $sql = "SELECT id FROM studentlogin WHERE email='$email' AND token='$token'";
    ...
    if (mysqli_num_rows($results) > 0) {
        ...
        $sql = "UPDATE studentlogin SET token='' WHERE email='$email'";
        $results = mysqli_query($db, $sql);
        $sql = "UPDATE studentlogin SET passwords='$newPasswordEncrypted' WHERE email='$email'";
        $results = mysqli_query($db, $sql);
        ?>
<!DOCTYPE html>
<html lang="en">
